  ...
  if (keywords$FCSversion == "2" ||
    ...
    if (datatype == 'F' || datatype == 'I') {
      ...
      if (compen == "internal") {
        ...
        spill = keyword(fcs_raw)$SPILL #FCS3.0 keyword
		if (is.null(spill) == TRUE) #FCS3.1 keyword
			spill = keyword(fcs_raw)$SPILLOVER
  ...
  if (keywords$FCSversion == "2" ||
    ...
    if (datatype == 'F') {
      ...
      if (compen == "internal") {
        ...
        spill = keyword(fcs_raw)$SPILL #FCS3.0 keyword
    		if (is.null(spill) == TRUE) { #FCS3.1 keyword
  				print(paste("SPILL is empty", spill))
  				# spill = keyword(fcs_raw)$SPILLOVER
          spill = keyword(fcs_raw)$`$SPILLOVER`
  				print("SPILLOVER is")
          print(spill)
